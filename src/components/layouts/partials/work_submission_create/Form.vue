...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            validateRequest() {
              if(!this.work_data.bill_id || !this.work_data.type_id || !this.work_data.material_id){
                Swal.fire({
                  icon: 'error',
                  title: 'Oops...',
            ...
            async submitBillSubmissionForm() {
                ...
                this.work_data = {submission_date: new Date().toISOString().substr(0, 10)}
            },
            fireSuccessMessage(online) {
                let message = online ? 'Work Submitted Successfully!' : 'Work Submission processing!';
                Swal.fire({
                    icon: "success",
                    html:
                        `${message}
